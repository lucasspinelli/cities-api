package com.lucaslevi.citiesapi.cities.Repository;
...
public interface CityRepository extends JpaRepository<City, Long> {
    ...
    Double distanceByPoints(Long cityIdA, Long cityIdB);

    @Query(value = "SELECT earth_distance(ll_to_earth(?1,?2), ll_to_earth(?3,?4)) as distance", nativeQuery = true)
    Double distanceByCube(double latA, double lonA, double latB, double lonB);
